...
export const FavoritesDrawer: React.FC<FavoritesDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#FFD600]/80 backdrop-blur-sm flex justify-end">
      
      <div className="w-full max-w-md bg-white border-l border-[#0A0A0A] text-[#0A0A0A] h-full flex flex-col justify-between shadow-none p-6">
        
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-[#0A0A0A]">
          <div className="flex items-center gap-2">
            <Heart className="w-5 h-5 text-[#0A0A0A] fill-[#5A5A40]" />
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-3">
          {favorites.length === 0 ? (
            <div className="text-center py-16 text-gray-500 space-y-2">
              <Heart className="w-10 h-10 mx-auto text-gray-500" />
              <p className="text-xs font-normal">Your saved listings portfolio is empty.</p>
            ...
            favorites.map((prop) => (
              <div
                key={prop.id}
                className="p-3 bg-[#FFD600] rounded-sm border border-[#0A0A0A] flex items-center justify-between gap-3 group"
              >
                <img
                  src={prop.mainImage}
                  alt={prop.title}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h4 
                    ...
                    className="text-xs font-sans font-black uppercase tracking-tighter text-gray-800 hover:text-[#0A0A0A] cursor-pointer truncate"
                  >
                    {prop.title}
                  </h4>
                  <span className="text-xs font-mono text-gray-700 font-bold block mt-0.5">
                ...
                <button
                  ...
                  className="p-2 rounded-sm hover:bg-white text-gray-500 hover:text-red-400 transition-colors"
        ...
        {favorites.length > 0 && (
          <div className="pt-4 border-t border-[#0A0A0A] space-y-3">
            ...
            <a
              ...
              className="w-full py-3 rounded-sm bg-[#0A0A0A] hover:bg-[#333333] text-[#0A0A0A] font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-none"
            >
